...
$$
g(x, y) = f(x, y) - \bar{f}(x,y)\\h(x, y) = f(x, y) + g(x, y)
$$
...
(1. 입력 영상의 엣지 부분에서의 그레이스케일 값의 변화)
(2. 원본 함수 f(x)를 스무딩한 결과)
(3. f(x)에서 스무딩한 것을 뺀 결과)
(4. g(x)를 f(x)와 더한 최종 결과 영상)
...
(1. 입력 영상에서 엣지 부분의 그레이스케일 값의 변화)
(2. f(x)를 x축에 대해 미분한 결과)
(3. f’(x)를 x축에 대해 미분한 결과)
(4. f(x)에 이차 미분 함수 f’’(x)를 빼준 엣지가 강조된 날카로운 결과 영상)
...
(입력 영상) (4방향 마스크를 사용한 결과) (8방향 마스크를 사용한 결과)
...
$$
∇_{N \sim W}I,\ c_{N \sim W}
$$

두 값을 이용하여 갱신됨을 의미한다. λ는 픽셀 값의 변화량을 결정짓는 상수이며, 0 ≤ λ ≤ 0.25를 만족해야 한다.
